...
export function ExamWorkspace({ items, expectedMarks, totalMarks, paperUrl, reportUrl, onChange }: {
  ...
  return <Card>
    ...
    <CardContent className="grid gap-5">
      {!items.length ? <div className="flex flex-col gap-3 rounded-lg border bg-muted/30 p-4 sm:flex-row sm:items-end"><Field className="flex-1"><FieldLabel htmlFor="workspace-checkpoints">Quick setup</FieldLabel><Input id="workspace-checkpoints" type="number" min="1" max="30" value={checkpointCount} onChange={(event) => setCheckpointCount(event.target.valueAsNumber)} /></Field><Button variant="outline" onClick={createCheckpoints}><Sparkles />Create equal checkpoints</Button></div> : null}
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_7rem_auto] sm:items-end">
        ...
        <Button onClick={add} disabled={remainingMarks <= 0}><Plus />Add</Button>
        <FieldError className="sm:col-span-full">{error}</FieldError>
      </div>
      {items.length ? <>
        <div className="grid grid-cols-3 gap-3"><div className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">Complete</p><p className="text-xl font-semibold tabular-nums">{doneMarks}/{mappedMarks}</p></div><div className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">Flagged</p><p className="text-xl font-semibold tabular-nums">{flagged}</p></div><div className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">Unmapped</p><p className="text-xl font-semibold tabular-nums">{remainingMarks}</p></div></div>
        ...
        <div className="grid gap-2">{items.map((item) => <div key={item.id} className="grid gap-3 rounded-lg border p-3 md:grid-cols-[minmax(0,1fr)_8rem_9rem_auto] md:items-center">
          <button type="button" className="min-w-0 text-left" onClick={() => update(item.id, { status: nextStatus[item.status] })}><span className={item.status === "done" ? "font-medium line-through text-muted-foreground" : "font-medium"}>{item.label}</span><Badge className="ml-2" variant="outline">{item.marks} marks</Badge><span className="mt-1 block text-xs text-muted-foreground">Click to advance status</span></button>
